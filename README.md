...
Cryptocurrency has boomed in popularity over the past few years. While Bitcoin was the first and remains the best-known cryptocurrency, Ethereum may overtake Bitcoin as the most valuable cryptocurrency sometime in the future. Predicting the price of a financial asset is very difficult because, in general, the price is determined by numerous variables that change over time. A multivariate time series model takes several factors into account. In this project, I build models to predict Ether price based on blockchain information by using Recurrent Neural Network LSTM and comparing it with traditional ML algorithms.
